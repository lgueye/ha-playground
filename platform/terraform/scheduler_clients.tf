...
resource "digitalocean_droplet" "scheduler_producer_01_droplet" {
  ...
  tags = ["${var.target_env}","${var.scheduler_client_role}","${var.scheduler_producer_role}","${var.discovery_client_role}"]
}

resource "ansible_host" "scheduler_producer_droplet" {
  inventory_hostname = "${digitalocean_droplet.scheduler_producer_01_droplet.name}"
  groups = ["${var.target_env}","${var.discovery_client_role}","${var.scheduler_client_role}","${var.scheduler_producer_role}"]
  vars {
