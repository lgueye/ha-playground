...
resource "digitalocean_tag" "discovery_server_role" {
  name = "${var.discovery_server_role}"
}
...
resource "digitalocean_droplet" "discovery_server_01_droplet" {
  ...
  tags = ["${digitalocean_tag.target_env.name}","${digitalocean_tag.discovery_server_role.name}"]
}
...
resource "ansible_host" "discovery_server_01_droplet" {
    ...
    groups = ["${var.target_env}","${var.discovery_server_role}","${var.primary_datacenter_role}"]
    vars {
...
resource "ansible_host" "discovery_server_02_droplet" {
    ...
    groups = ["${var.target_env}","${var.discovery_server_role}","${var.fallback_datacenter_role}"]
    vars {
...
resource "ansible_host" "discovery_server_03_droplet" {
    ...
    groups = ["${var.target_env}","${var.discovery_server_role}","${var.ternary_datacenter_role}"]
    vars {
