...
resource "digitalocean_tag" "cache_server_role" {
  name = "${var.cache_server_role}"
}
...
resource "digitalocean_droplet" "cache_server_01_droplet" {
  ...
  tags = ["${digitalocean_tag.target_env.name}","${digitalocean_tag.discovery_client_role.name}","${digitalocean_tag.cache_server_role.name}"]
}

resource "ansible_host" "cache_server_01_droplet" {
    inventory_hostname = "${digitalocean_droplet.cache_server_01_droplet.name}"
    groups = ["${var.target_env}","${var.discovery_client_role}","${var.cache_server_role}","${var.primary_datacenter_role}"]
    vars {
...
resource "ansible_host" "cache_server_02_droplet" {
    ...
    groups = ["${var.target_env}","${var.discovery_client_role}","${var.cache_server_role}","${var.fallback_datacenter_role}"]
    vars {
...
resource "ansible_host" "cache_server_03_droplet" {
    ...
    groups = ["${var.target_env}","${var.discovery_client_role}","${var.cache_server_role}","${var.ternary_datacenter_role}"]
    vars {
